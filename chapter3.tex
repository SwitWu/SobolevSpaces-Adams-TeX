...
\begin{example}
  ...
  there can exist no $\phi \in C^1(\bar{\Omega})$ such that
  $\|u-\phi\|_{1, p, \Omega}<\varepsilon$. To see this, suppose there exists such a $\phi$.
  If $L=\{(x, y):-1 \leq x \leq 0,0 \leq y \leq 1\}$ and $R=\{(x, y): 0 \leq x \leq 1$, $0 \leq y \leq 1\}$, then $\bar{\Omega}=L \cup R$.
  ...
  \[
  \begin{aligned}
  1-2 \varepsilon<\Phi(b)-\Phi(a)=\int_a^b \Phi^{\prime}(x) d x & \leq \int_{\bar{\Omega}}\left|D_x \phi(x, y)\right| d x d y \\
  ...
  and $m, C^k(\bar{\Omega})$ is dense in $W^{m, p}(\Omega)$ provided $1 \leq p<\infty$.
\end{example}


\begin{para}[The Segment Condition]
  We say that a domain $\Omega$ satisfies the segment condition if every $x \in$ bdry $\Omega$
  has a neighbourhood $U_x$ and a nonzero vector $y_x$ such that if $z \in \bar{\Omega} \cap U_x$, 
...
\begin{proof}
  ...
  found via Lemma 3.16 since $\operatorname{supp}\left(u_0\right) \subset V_0 \subset\subset \Omega$.
...
\begin{para}[$\bm{(m, p')}$-Polar sets]
  ...
  in $F(\operatorname{supp}(T) \subset F)$ provided that $T(\phi)=0$ for every
...
\begin{proof}
  ...
  & \leq 0+2 \int_{\left\{x \in \Omega: \psi(x) \neq \chi_K(x) \operatorname{sgn} \overline{u(x)\}}\right.}|u(x)| d x<\varepsilon .
  \end{aligned}
  \]
...
\begin{para}
  ...
  \]
  which in turn implies that $W^{-1 \cdot q^{\prime}}\left(\mathbb{R}^n\right) \subset W^{-m, p^{\prime}}\left(\mathbb{R}^n\right)$. If also $p \geq 2 n /(n+m-1)$, then $q^{\prime} \leq p$ and so by Lemma $3.36, \Omega^c$ is $(1, p)$-polar if it is $\left(m, p^{\prime}\right)$-polar. Note that $2 n /(n+m-1)<2$ if $m>1$. If, on the other hand, $(m-1) p \geq n$, then $m p>n$, and, as pointed out in Paragraph $3.25, \Omega^c$ cannot be $\left(m, p^{\prime}\right)$-polar unless it is empty, in which case it is trivially $(1, p)$-polar.
  
  The only values of $p$ for which we do not know that the $\left(m, p^{\prime}\right)$-polarity of $\Omega^c$ implies $(1, p)$-polarity and hence is equivalent to the identity of $W^{m, p}(\Omega)$ and $W_0^{m, p}(\Omega)$, are given by $1 \leq p \leq \min \{n /(m-1), 2 n /(n+m-1)\}$
\end{para}
