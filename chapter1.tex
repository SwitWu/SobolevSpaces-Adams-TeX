...
\begin{para}[Reflexive Spaces]
  ...
  \]
  Since $\left|J x\left(x^{\prime}\right)\right| \leq\left\|x^{\prime}\right\|_{X^{\prime}}\|x\|_{X}$, we have
  \[
  \left\|J x\right\|_{X^{\prime \prime}} \leq\|x\|_{X}
  \]
  However, the Hahn-Banach Extension Theorem assures us that for any $x \in X$ we can find $x^{\prime} \in X^{\prime}$ such that $\left\|x^{\prime}\right\|_{X^{\prime}}=1$ and $x^{\prime}(x)=\|x\|_{X}$. Therefore $J$ is an isometric isomorphism of $X$ into $X^{\prime \prime}$.
...
\begin{theorem}
  A Banach space is reflexive if and only if its closed unit ball $\overline{B_1(0)}=\{x \in X:\|x\|_{X} \leq 1\}$ is weakly sequentially compact.
\end{theorem}
...
\begin{para}[Uniform Convexity]
  ...
  that if $x, y \in X$ satisfy $\|x\|_{X}=\|y\|_{X}=1$ and $\|x-y\|_{X} \geq \varepsilon$, then $\|(x+y) / 2\|_{X} \leq$ $1-\delta(\varepsilon)$.
...
\begin{para}[Operators]
  ...
  Every compact operator is bounded. Every bounded linear operator is continuous. Therefore, every compact linear operator is completely continuous. The norm of a linear operator $f$ is $\sup \{\|f(x)\|_{Y}:\|x\|_{X} \leq 1\}$.
\end{para}
...
\begin{para}[Imbeddings]
  ...
  \[
  \|I x\|_{Y} \leq M\|x\|_{X}, \quad x \in X \text {. }
  \]
...
\begin{proof}
  ...
  then there exists $M$ such that $\|\phi\|_{C^{0, \lambda}(\overline{\Omega})} \leq M$
